...
class Module(core.module.Module):
    def __init__(self, config, theme):
        ...
        # Parameters
        self.__work_period = int(self.parameter("work", 60))  # trabalha 1h
        self.__break_period = int(self.parameter("break", 10))  # descansa 10 min
        self.__time_format = self.parameter("format", "%m:%s")
        self.__notify_cmd = self.parameter("notify", "")

        # Time format flags
        self.display_seconds_p = "%s" in self.__time_format
        self.display_minutes_p = "%m" in self.__time_format
    ...
    def text(self, widget):
        return "Horas: {} | {}".format(self.work_cycles, self.__text)
    ...
    def timer_reset(self, widget):
        if self.pomodoro["state"] in ["ON", "PAUSED"]:
            self.pomodoro = {"state": "OFF", "type": ""}
            self.remaining_time = datetime.timedelta(minutes=self.__work_period)

    def state(self, widget):
        state = [self.pomodoro["state"].lower()]
        if self.pomodoro["state"] in ["ON", "OFF"]:
